...
//creating the product card and appending it to the various sections based on whether it is an accessory or not
function productRender(thumbnailUrl, titleName, brandName, productPrice, isAccessory)
{
  ...
  productCard.onmouseover = function() {
    productCard.style.boxShadow = '0 2px 10px rgba(0, 0, 0, 0.7)';
  }

  productCard.onmouseout = function() {
    productCard.style.boxShadow = '0 2px 10px rgba(0, 0, 0, 0.3)';
  }

  //creating the thumbnail for product card and appending it
  var thumbnail = document.createElement('img');
  thumbnail.className = "thumbnail";
  thumbnail.src = thumbnailUrl;
  productCard.appendChild(thumbnail);
 
  //creating the info part of the card and appending i
  var details = document.createElement('div');
  details.className = "details";
  var name = document.createElement('h3');
  name.innerHTML = titleName;
  var brand = document.createElement('p');
  brand.className = "brand-name";
  brand.innerHTML = brandName;
  var price = document.createElement('p');
  price.className = "product-price";
  price.innerHTML = "Rs " + productPrice;
  details.appendChild(name);
  details.appendChild(brand);
  details.appendChild(price);
  productCard.appendChild(details);

  //allocating the prooduct card to sections based upon condition check
  if(isAccessory == true){
 
    accessory.appendChild(productCard);
  }
  else
     {
...
{
  productRender(productList[pos].preview, productList[pos].name, productList[pos].brand, productList[pos].price, productList[pos].isAccessory); 
}
